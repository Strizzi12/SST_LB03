...
namespace VE
{
    public class TransactionWrapper
	{
		...
		public static bool Intf_transfer(int custId, int tmpFromAccID, int tmpToAccID, float tmpValue, int currency)
		{
			...
			if(currency == 0)
				return xmlcontroler_transferMoney(tmpFromAccID, tmpToAccID, tmpValue);
			else
				return xmlcontroler_transferMoney(tmpFromAccID, tmpToAccID, (float)(CurrencyExchangeWrapper.Intf_exchange(tmpValue, currency, 0)));
		}
		...
		{
			return xmlcontroler_withdrawMoney(tmpAccID, tmpValue);
		}
		...
		{
			return xmlcontroler_depositMoney(tmpAccID, tmpValue);
		}
